...
if(!isset($idevento) || $idevento == '' || $idevento == '#'){
	...
	//QUERY
	include_once('perguntas_busca_query.php');
	...
	if(mysql_num_rows($rs) || $disparabusca == 'ok'){
		...
		if(mysql_num_rows($rs) == 0 && $disparabusca == 'ok'){
			...
			$_SESSION['MSG'] = "Não foram encontradas perguntas correspondentes à sua busca.";
		...
		while($rsDados = mysql_fetch_array($rs)){
			...
			$sqlR = "SELECT * FROM respostas WHERE idpergunta = '".$rsDados['idpergunta']."'";	
			$rsR = mysql_query($sqlR);
			
			$tpl->assign('qtdRespostas', mysql_num_rows($rsR));		
		}
	}else{
...
if(session_is_registered('INSERIDO')){
	$tpl->assignGlobal('mensagem','alert("Pergunta inserida com sucesso!");');
