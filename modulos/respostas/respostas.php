...
if(!isset($idevento) || $idevento == '' || $idevento == '#'){
	...
	//QUERY
	include_once('respostas_busca_query.php');
	...
	if(mysql_num_rows($rs) || $disparabusca == 'ok'){
		...
		if(mysql_num_rows($rs) == 0 && $disparabusca == 'ok'){
			...
			$_SESSION['MSG'] = "Não foram encontradas repostas correspondentes à sua busca.";
		...
		while($rsDados = mysql_fetch_array($rs)){
			...
			$tpl->assign('idresposta',$rsDados['idresposta']);
...
if(session_is_registered('INSERIDO')){
	$tpl->assignGlobal('mensagem','alert("Resposta inserida com sucesso!");');
