...
function uploadMultiploFlame() 
{
	...
	fileTypes = [{description:"Arquivos", extension:"*.*"}];

	//ARQUIVO EM PHP QUE IRA CONTROLAR O UPLOAD
	if (uploadUrl == null) 
	{
		uploadUrl = "arquivos_upload.php?id="+_level0.id+"&tipo="+_level0.tipo;
	}
...
function uploadQueueChanged (eventObj:Object) 
{
	switch (eventObj.type) 
	{
		...
		case "complete":
			...
			getURL("javascript:window.location.href = 'arquivos.php?id="+_level0.id+"&tipo="+_level0.tipo+"';");
			break;
