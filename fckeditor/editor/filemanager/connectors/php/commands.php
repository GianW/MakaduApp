	...
	if ( isset( $_FILES['NewFile'] ) && !is_null( $_FILES['NewFile']['tmp_name'] ) )
	{
		...
		if ( !$sErrorNumber && IsAllowedExt( $sExtension, $resourceType ) )
		{
			...
			while ( true )
			{
				...
				{
					if($oFile['type'] == 'image/pjpeg' || $oFile['type'] == 'image/jpeg' || $oFile['type'] == 'image/gif' || $oFile['type'] == 'image/png'){
						//include_once('../../../../../../fl_adm/_config.php');
						$maxWidth = 1000;
						$maxHeight = 1000;
						$picInfos = getimagesize($oFile['tmp_name']);
						$width = $picInfos[0];
						$height = $picInfos[1];
						...
						switch ($oFile['type']) {
							case 'image/pjpeg':
								...
								imagejpeg($canvas,$sFilePath,100);				
							break;
							case 'image/jpeg':
								$img = imagecreatefromjpeg($oFile['tmp_name']);
								$canvas = imagecreatetruecolor ($nWidth,$nHeight);
